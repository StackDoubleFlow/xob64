...
pub fn compile_instr(
    ...
    match arm_instr.op() {
        Op::STP => {
            ...
            let base_reg = match base_reg_translation {
                ...
                RegTranslation::Indirect(indirect_idx) => {
                    if double_indirect {
                        ...
                        // We need to load the offset into a register that isn't the scratch, and we need to pick a register that isn't going to be the store value of either reg1 or reg2.
            ...
            make_store(ass, reg1, base_reg, offset)?;
            let incr = if get_reg_class(reg1).0 == RegClass::GPR32 {
                4
            } else {
                8
            };
            make_store(ass, reg2, base_reg, offset + incr)?;
